...
- **Ground scopes are complete and literal.** `.frc-ground-squadron`, `.frc-ground-field` and `.frc-ground-paper` each declare the same 35 semantic aliases as literal values — never `var()`, never only at the root. `npm run ds:audit` fails on drift; `/_ds` proves the computed values in the browser.
...
- `/_ds` in `npm run dev` — live proofs: alias resolution in all three grounds, no rendered gold on paper, all four transitions, every animation gated, static end state, no neutral white in chrome, all fifteen components mounted from source.
