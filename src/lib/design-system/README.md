...
- `npm run ds:audit:controls` — the negative controls. It breaks each of checks 16 through 27 in turn, asserts the right failure fires, and restores the file. It refuses to run on a dirty tree. A check that has never failed has not been tested.
