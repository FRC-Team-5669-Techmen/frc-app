...
- `npm run ds:audit:controls` — the controls. It breaks each of checks 16 through 27 in turn, asserts the right failure fires, and restores the file. It refuses to run on a dirty tree. A check that has never failed has not been tested. One control is **positive**: check 17 locates its hashed slice from the `INHERITANCE, NON-NEGOTIABLE` anchor rather than by first occurrence, because the standard quotes both slice markers, and 17c proves that a quote above the block does not move the slice.
